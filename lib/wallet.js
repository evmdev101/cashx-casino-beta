...
(function (root) {
  ...
  async function waitForWalletProvider(timeoutMs = DEFAULT_PROVIDER_WAIT_MS) {
    const start = Date.now();
    if (hasWallet()) return eth();

    // Some wallets inject after DOM load; MetaMask also emits this init event in many contexts.
    const initialized = new Promise(resolve => {
      const handler = () => {
        try { root.removeEventListener('ethereum#initialized', handler); } catch (_) {}
        resolve();
      };
      try { root.addEventListener('ethereum#initialized', handler, { once: true }); } catch (_) { resolve(); }
      // Fallback: don't block forever if the event never fires.
      setTimeout(resolve, Math.min(350, timeoutMs));
    });
  ...
  async function listWalletProviders(waitMs = 250) {
    const timeoutMs = Math.max(0, Number(waitMs || 0));
    await waitForWalletProvider(timeoutMs);
  ...
  async function reconnect() {
    ...
    if (!hasWallet()) {
      await waitForWalletProvider(DEFAULT_PROVIDER_WAIT_MS);
      if (!hasWallet()) return null;
    }
